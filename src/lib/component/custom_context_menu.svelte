<script lang="ts">
  ...
  function rightClickContextMenu(e: MouseEvent) {
    ...
    if (e.target instanceof HTMLElement) {
      contextState.target = e.target;
    } else {
      contextState.target = null;
    }
  }
...
<svelte:window on:contextmenu = {(e) => {
  if(e.target instanceof HTMLElement) {
    if((e.target as HTMLElement).classList.contains("show-custom-context-menu")) {
      e.preventDefault();
